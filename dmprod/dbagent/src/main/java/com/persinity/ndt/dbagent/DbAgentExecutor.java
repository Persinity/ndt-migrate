...
public class DbAgentExecutor {
    ...
    public ClogAgent<Function<RelDb, RelDb>> clogAgentInstallMount(
            final DbAgentFactory<Function<RelDb, RelDb>> dbAgentFactory, final RelDb ndtDb, final boolean isSrcClog) {

        final ClogAgent<Function<RelDb, RelDb>> clogAgent = dbAgentFactory.dispatchClogAgent(ndtDb, isSrcClog);
        processor.process(clogAgent.clogUmount(), ndtDb, safeDropTableExceptionHandler);

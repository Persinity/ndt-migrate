...
public class NdtController {
    ...
    public static void main(String[] args) {
        NdtController ndtController = null;
        try {
            log.info("{}", BuildInfo.getInstance());
            ndtController = createFromConfig(NdtControllerConfig.DEFAULT_CONFIG_FILE_NAME);
            final NdtShutdownHook shutdownHook = new NdtShutdownHook(ndtController);
     ...
     */
    public void ndtCleanupOnFailure(final String context) {
        // first close view to stop user interaction only output
        view.close();

        final NdtDisableAndUninstall ndtDisableAndUninstall_;
        synchronized (this) {
            ndtDisableAndUninstall_ = ndtDisableAndUninstall;
            ndtDisableAndUninstall = null;
        }

        if (ndtDisableAndUninstall_ != null) {
            script.sigStop();
            ...
            try {
                view.logNdtMessage(format("Aborting on {}...", context));
            } catch (RuntimeException _) {
                // silence
            }
            try {
                ndtDisableAndUninstall_.forceWork();
            } catch (RuntimeException e) {
                log.warn(e, "Cleanup on {} failed!", context);
            }
        }
    }
    ...
    private void defineSteps() {
        ...
        script.addStep(ndtDisableAndUninstall);
    }
